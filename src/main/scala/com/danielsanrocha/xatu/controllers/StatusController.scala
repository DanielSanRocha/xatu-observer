...
import com.danielsanrocha.xatu.models.internals.{RequestId, Status, TimedCredential}
...
class StatusController(
  ...
  get("/status") { request: Request =>
    ...
    response.ok(Status(apiObserverManager.status(), logServiceObserverManager.status(), serviceObserverManager.status(), logContainerManager.status()))
  }
}
