...
class ServiceRepositoryImpl(implicit client: Database, implicit val ec: scala.concurrent.ExecutionContext) extends ServiceRepository {
  ...
  override def create(service: NewService): Future[Long] = {
    client.run((services.map(s => (s.name, s.logFileDirectory, s.logFileRegex, s.pidFile)) returning services.map(_.id)) += ((service.name, service.logFileDirectory, service.logFileRegex, service.pidFile))) map { x =>
      x.toLong
    }
  }
